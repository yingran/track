...
function readConf(hostname, basepath, pathname, req, res){
    ...
    if (match&&match[0]){
        ...
        path.exists(confpath, function(isExists){
            var conf=[], requires=[], yuiRequires=[], r1, r2, i, j, len, len2, fname, data=[], taskData=[], parsed = {}, yuiData={js:"", css:""}, compressed;
            if (isExists){
                try {
                ...
                for (i=0; i<len; i++){
                    r1 = getModule("task/" + conf[i], devpath, res, parsed, res);
                    r2 = [];
                    len2 = r1.length;
                    for (j=0; j<len2; j++){
                ...
                if (yuiData.css && yuiData.css.length > 0){
                    getYuiCss(yuiData.css, hostname, project, minpath, filename);
                }
...
function getModule(mname, devpath, res, parsed, res){
    mname = mname.trim();
    if (yuiModules[mname]){
        return [];
    }
    ...
    try{
        ...
        temp = [];
        for (i=0; i<len1; i++){
            temp2 = getModule(requires[i], devpath, res, parsed, res);
            len2 = temp.length;
            for (j=len2-1; j>=0; j--){
...
function getYuiCss(css, hostname, project, minpath, filename){
    ...
    css.forEach(function(file) {
        str.push("@import url(http://" + hostname + file.replace("/data/lib/node_modules/", "/public/") + ");");
    });
    fs.writeFileSync((minpath + '/' + filename), str.join("\n"));
